...
const Home = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gold-800 via-gold-900 to-black">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 w-full z-50 bg-black/30 backdrop-blur-lg shadow-md py-4 px-8 flex justify-between items-center">
        ...
        <div className="space-x-8 text-lg">
          ...
          <Link
            to="/register"
            className="text-black bg-yellow-100 px-4 py-2 rounded-lg shadow hover:bg-green-500 transition"
          >
            Register
          </Link>
        </div>
      ...
      {/* Hero Section */}
      <div className="relative w-full h-[90vh] mt-16 overflow-hidden">
        {images.map((img, idx) => (
          <div
            key={idx}
